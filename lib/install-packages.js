...
function installPackages(config, context, projectDir, done) {
  var update = config.update_cache;
  var cachier = context.cachier('bower');
  ...
  bowerHash(projectDir, function (err, hash) {
    ...
    var dest = path.join(projectDir, 'bower_components');

    // try for an exact match
    cachier.get(hash, dest, function (err) {
      if (!err) {
        context.comment('restored bower_components from cache');
